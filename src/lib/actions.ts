...
export async function discoverTrends(
  ...
  const discoverFn = async (validatedInput: TrendingContentInput): Promise<TrendingContentOutput> => {
      const [trendingHashtags, viralContentIdeas, trendingThumbnailIdeas] = await Promise.all([
          getTrendingHashtags(validatedInput),
          getViralContentIdeas(validatedInput),
          getTrendingThumbnailIdeas(validatedInput)
      ]);
      return { trendingHashtags, viralContentIdeas, trendingThumbnailIdeas };
  }
  try {
      const result = await discoverFn(input);
      return { status: 'success', data: result };
  } catch (e: any) {
        return handleAIError(e);
  }
